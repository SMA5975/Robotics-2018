...
public class Robot extends IterativeRobot {
    ...
    private Gyro gyro; //pull for our code

    double Kp = 0.03;

   /* public Robot() {//move to inits?
    	super();
    ...
    public void autonomousPeriodic() {// fun numbers going up inexplicably
        gyro.reset();
        while (isAutonomous()) {
            double angle = gyro.getAngle(); // get current heading
            System.out.println(angle%360);
            SmartDashboard.putNumber("Gyro", angle%360);
            //myRobot.drive(0.2, -angle*Kp); // drive towards heading 0, 0.2 probably slowest possible to run 
            Timer.delay(0.004);
        }
